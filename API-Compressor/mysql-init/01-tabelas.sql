...
-- -----------------------------------------------------
CREATE TABLE
  IF NOT EXISTS `compressor_db`.`comandos` (
    `id` INT NOT NULL AUTO_INCREMENT,
    ...
    CONSTRAINT `fk_comandos_compressor1` FOREIGN KEY (`compressor_id`) REFERENCES `compressor_db`.`compressor` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
  ) ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `comando_agendado`
-- -----------------------------------------------------
CREATE TABLE
  IF NOT EXISTS comando_agendado (
    ...
    ultimo_disparo_inicio DATETIME NULL,
    ultimo_disparo_fim DATETIME NULL,
    -- estado
    ativo TINYINT (1) NOT NULL DEFAULT 1,
    descricao VARCHAR(255),
    CONSTRAINT fk_ag_compressor FOREIGN KEY (compressor_id) REFERENCES compressor (id)
  );

-- -----------------------------------------------------
-- Table `agendamento_dias_semana`
-- usada pelo @ElementCollection List<DayOfWeek>
-- -----------------------------------------------------
CREATE TABLE
  IF NOT EXISTS agendamento_dias_semana (
    agendamento_id INT NOT NULL,
    dia_semana VARCHAR(20) NOT NULL,
    CONSTRAINT fk_dias_semana_agendamento FOREIGN KEY (agendamento_id) REFERENCES comando_agendado (id) ON DELETE CASCADE
  );